...
def is_leave_valid(leave_data):
    messages = []
    misc_leave_types = [str(id) for id in LeaveType.objects.filter(rule_set__name='miscellaneous_leave').values_list('id', flat=True)]
    ...
    elif leave_data['leave_type'] == str(sick_leave_type.id) and leave_data['file'] is None:
